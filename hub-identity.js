...
function removeUnapprovedVisuals(html) {return String(html || '').replace(/<picture\b[^>]*>[\s\S]*?<\/picture>/gi, '').replace(/<svg\b[^>]*>[\s\S]*?<\/svg>/gi, '').replace(/<(?:object|embed)\b[^>]*>(?:[\s\S]*?<\/(?:object|embed)>)?/gi, '').replace(/<img\b[^>]*>/gi, '').replace(/<source\b[^>]*>/gi, '').replace(/\s+poster=(?:"[^"]*"|'[^']*'|[^\s>]+)/gi, '').replace(/background(?:-image)?\s*:\s*url\([^;}]+\)\s*;?/gi, 'background-image:none;');}
function injectOfficialLogo(html) {let output = String(html || '').replace(/<span\s+class=["'][^"']*\blmi-wordmark-mark\b[^"']*["'][^>]*>[\s\S]*?<\/span>/gi, '').replace(/<div\s+class=["'][^"']*\bmark\b[^"']*["'][^>]*>\s*LMI\s*<\/div>/gi, '').replace(/\.lmi-wordmark-mark\{[^}]*\}/gi, '');if (/class=["'][^"']*\blmi-wordmark\b/i.test(output)) return output.replace(/(<a\b[^>]*class=["'][^"']*\blmi-wordmark\b[^"']*["'][^>]*>)/i, `$1${OFFICIAL_LOGO_MARKUP}`);if (/class=["'][^"']*\bidentity\b/i.test(output)) return output.replace(/(<div\b[^>]*class=["'][^"']*\bidentity\b[^"']*["'][^>]*>)/i, `$1${OFFICIAL_LOGO_MARKUP}`);if (/class=["'][^"']*\bbrand\b/i.test(output)) return output.replace(/(<div\b[^>]*class=["'][^"']*\bbrand\b[^"']*["'][^>]*>)/i, `$1${OFFICIAL_LOGO_MARKUP}`);return output.replace(/(<body(?:\s[^>]*)?>)/i, `$1${OFFICIAL_LOGO_MARKUP}`);}
export function enforceOfficialHubIdentity(relativePath, html) {const normalized = String(relativePath || '').replace(/^\/+/, '');if (!HUB_DRAFT_PATTERN.test(normalized)) return html;let output = injectOfficialLogo(removeUnapprovedVisuals(html));if (!output.includes('id="lmi-official-identity-guard"')) output = output.replace(/<\/head>/i, `${IDENTITY_CSS}</head>`);return output;}
