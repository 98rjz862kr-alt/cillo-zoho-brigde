...
function adminLoginPage() {
  ...
  <style>body{font-family:system-ui;margin:40px;max-width:560px}input,button{font:inherit;padding:10px;margin:6px 0;width:100%;box-sizing:border-box}button{background:#111827;color:#fff;border:0;border-radius:8px}</style></head><body>
...
function requireAdmin(req, res, query, body) {
  if (!isAuthorized({ headers: req.headers, query, body })) {
    sendJson(res, { error: 'Unauthorized. Provide x-admin-password header, password query, or password form field.' }, 401);
    return false;
  }
  return true;
}
...
async function handleRequest(req, res) {
  ...
  if (req.method === 'GET' && (url.pathname === '/openapi.yaml' || url.pathname === '/docs/openapi.yaml')) {
    return sendText(res, readFileSync(path.join(rootDir, 'docs', 'openapi.yaml'), 'utf8'), 200, { 'content-type': 'application/yaml; charset=utf-8' });
  }
