...
for (const file of expectedHubFiles) {
  if (!actualFiles.has(file)) throw new Error(`Hub file missing: ${file}`);
}
...
for (const draft of hubDrafts) {
  ...
  if (!/<meta[^>]+name=["']robots["'][^>]+noindex/i.test(html)) {
    throw new Error(`Robots noindex missing: ${draft.relativePath}`);
  }
  if (!html.includes('id="lmi-hub-final-script"')) {
    throw new Error(`Hub finalisation layer missing: ${draft.relativePath}`);
  }

  const pageNumber = Number.parseInt(draft.relativePath.split('/').pop().slice(0, 2), 10);
  if (pageNumber >= 2 && pageNumber <= 32) {
    if (!html.includes('data-lmi-runtime="premium-v2"')) {
      throw new Error(`Premium runtime missing: ${draft.relativePath}`);
    }
    if (!html.includes('id="lmi-premium-runtime-script"')) {
      throw new Error(`Premium script missing: ${draft.relativePath}`);
    }
  }
...
if (!decoratedPage.includes('lmi-route-breadcrumb')) {
  throw new Error('Hub breadcrumb layer is missing');
}
if (!decoratedPage.includes('lmi-runtime-pager')) {
  throw new Error('Hub previous/next navigation is missing');
}

console.log(`Validated ${hubDrafts.length} Hub LMI draft pages with protected routing and premium presentation.`);
