...
def machine() -> HTMLResponse:
    ...
    html = html.replace(
        "</head>",
        "<link rel='icon' href='/lmi-brand.svg' type='image/svg+xml'>"
        "<link rel='stylesheet' href='/lmi-theme.css'></head>",
    )
    html = html.replace(
        '<div class="brand"><div class="brandmark">LMI</div><div><b>CDM MACHINE</b><small>Contrôle & déploiement</small></div></div>',
        '<div class="brand"><img class="brand-logo" src="/lmi-brand.svg" alt="Les Mots Images — Le Verbe par l’Image"><div class="brand-copy"><b>CDM MACHINE</b><small>Contrôle & déploiement</small></div></div>',
    )
    html = html.replace(
        '<header class="topbar"><h1>Machine de contrôle & déploiement maîtrisé</h1>',
        '<header class="topbar"><div class="topbar-title"><span class="topbar-emblem">CDM</span><div><h1>Machine de contrôle & déploiement maîtrisé</h1><small>Les Mots Images — Le Verbe par l’Image.</small></div></div>',
    )
    html = html.replace(
        "</main>",
        "</main><footer class='lmi-footer'><strong>LES MOTS IMAGES</strong> · LE VERBE PAR L’IMAGE. · CDM</footer>",
    )
